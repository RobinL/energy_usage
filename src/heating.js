...
// Not going to use the following but may be useful at some later date:
// const heat_capacity_of_air_joules = 1200

//md`Temperature data available from [here](http://data.ceda.ac.uk/badc/ukcp09/data/gridded-land-obs/gridded-land-obs-daily/timeseries/mean-temperature), specifically (for where I live) [here](http://dap.ceda.ac.uk/thredds/fileServer/badc/ukcp09/data/gridded-land-obs/gridded-land-obs-daily/timeseries/mean-temperature/ukcp09_gridded-land-obs-daily_timeseries_mean-temperature_500000E_150000N_19600101-20161231.csv).  Note you need to be logged in.  I have downloaded and transformed data using [this](https://gist.github.com/RobinL/f9d478dadd70ec2dda67c1e9e91686f1) script`

// http://www.inference.org.uk/sustainable/book/tex/cft.pdf page 294
// conduction_loss_power = function (surface_area, temp_diff, average_u) {
//     return surface_area * temp_diff * average_u
// }

// ventilation_loss_power = function (volume, temp_diff, number_air_changes_per_hour) {
//     //12000 is the heat capacity of air
//     return 12000 * (number_air_changes_per_hour / 3600) * volume * temp_diff
// }
...
function convert_gas_units_to_kwh(amount, units) {
    if (units == "kwh") {
        return amount
    }
    if (units == "imperial_units") {
        return amount * utils.KWH_PER_IMPERIAL_GAS_UNIT
    }

    if (units == "metric_units") {
        return amount * utils.KWH_PER_METRIC_GAS_UNIT
    }
}
...
function heating_kwh_per_day(annual_usage_from_energy_bill, thermostat_current, thermostat_future, num_in_household, units = "kwh") {
    let kwh_per_year =  convert_gas_units_to_kwh(annual_usage_from_energy_bill, units)
    let kwh_per_day = utils.convert_time_period(kwh_per_year, 'years', 'days')
